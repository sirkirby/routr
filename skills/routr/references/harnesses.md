...
| Kiro (`kiro`) | `chat --trust-all-tools` (measured, Kiro CLI 2.24.1, 2026-09-26: no folder-trust dialog). Model: `--model <id>`; list with `kiro-cli chat --list-models`. No effort: `--effort` exists, but every model the account offered showed effort "n/a" in `/effort` and the flag was ignored silently | **Asks "Kiro is running in trust all tools mode" at every start**, with "No, exit" selected, while herdr reports it idle and ready: read the pane. "Yes, I accept" holds for that session; "Yes, and don't ask again" changes the user's Kiro settings. An arrow key sent in the same burst as enter was lost and chose "No, exit": move, read, then press enter. **An unknown `--model` id runs the default silently**: the footer (`kiro_default · <model> · ◔ 5%`) then shows no model. Headless (`--no-interactive`) ignores `--model` altogether ("failed to set model … Method not found"). Reads skills only from `~/.kiro/skills`, not `~/.agents/skills`. Quits with `/quit`. |
