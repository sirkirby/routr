...
| Can each harness be launched unattended in a herdr pane? | the orchestrator's own check of each result | 5/5 (Kiro added 2026-09-26: one run, a one-file task, verified from the pane and the file). Found on the way: folder-trust dialogs (Claude Code, Codex), shell start-up prompts that swallow the launch command, Antigravity having no skill mechanism and silently running its high variant when given both a model and an effort, Cursor's `--model` persisting as the account default, Kiro asking to confirm its trust-all-tools mode at every start (with an arrow key lost when sent together with enter), running its default on an unknown model id without a word, and ignoring `--effort` on every model offered |
...
| Are the usage numbers right? | each harness's own usage screen | Claude Code matches its usage page; Antigravity and Codex are read from the harness itself, and Kiro from its own `/usage` command. Kiro's figure is its own estimate and lags: minutes after a turn Kiro billed at 0.10 credits, `/usage` still read 0.00 (one observation, a Free plan). A flat reserve made a subscription look spent a day before its reset, so the reserve now shrinks toward the reset |
