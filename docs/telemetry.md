...
you. It stays off, even when turned on, while `ROUTR_TELEMETRY=0` or `DO_NOT_TRACK=1` is set, and always in CI.

When you turn it on, only rows recorded from then on are shared. Rows already in your ledger stay on your machine
unless you send them yourself with `routr telemetry send --all`.
...
|---|---|---|
| `day` | `2026-09-24` | the date the row was recorded; no time of day |
...
| `advised` | level, sure, work type, high risk, facts | what routr read: yes/no probabilities for its fixed questions |
...
your machine the first time you share. The install id is not derived from anything about you or your machine.
...
Run `routr feedback "please delete my telemetry"`. The note carries your install id, which is how the maintainers
find your rows, and they will delete them.
